...
import group3.vute.travellaos.databinding.DialogPlaceDetailsBinding;

public class PlaceDetailDialog extends Dialog {
    private final DialogPlaceDetailsBinding binding;
    private List<ReviewWithUser> listReview;

    @SuppressLint("SetTextI18n")
    public PlaceDetailDialog(@NonNull Context context, Place data) {
        super(context);
        binding = DialogPlaceDetailsBinding.inflate(LayoutInflater.from(context));
        AppDatabase db = AppDatabase.getInstance(getContext());

        // Load review data
        listReview = db.reviewDao().getByPlaceId(data.getId());
        ...
        if (user == null) {
            ...
            Review dataCheck = db.reviewDao().getReviewedPlace(user.getId(), data.getId());
            if (dataCheck != null) {
                ...
                binding.btnSend.setOnClickListener(v -> {
                    ...
                    try {
                        db.reviewDao().insert(new Review(user.getId(), data.getId(), null, (int) binding.ratingBar.getRating(), content, sdf.format(new Date())));
